...
if (isset($_GET['codAdm'])) {
    ...
    $sql = "delete from adm where codAdm = " . $_GET['codAdm'];
...
<section class="home-section">
    ...
    <div class="card-listar">
        <div class="card-body pesquisar">
            <form method="POST">
                <div class="input-group">
                    <div class="input-box">
                        <label for="nome">Nome do Adm</label>
        ...
        <div class="card-body">
            <h2 class="title-marca">Listagem Administrador</h2>
            <a href="cadAdm.php" class="btn-plus"><i class="fa-solid fa-plus"></i></a>
        </div>
    </div>
            ...
            <table class="tbl">
                ...
                <tbody>
                    <?php while ($linha = mysqli_fetch_array($resultado)) { ?>
                        <tr>
                            <td data-lable="ID"><?= $linha['codAdm'] ?></th>
                            ...
                            <td data-lable="Editar">
                                <a href="alterarAdm.php?codAdm=<?= $linha['codAdm'] ?>" class="botao-listar btn_edit"><i class="fa fa-pencil"></i></a>

                            </td>
                            <td data-lable="Excluir">
                                <a href="listarAdm.php?codAdm=<?= $linha['codAdm'] ?>" class="botao-listar btn_trash" onclick="return confirm('Confirmar exclusão?')"><i class="fa fa-trash"></i></a>

                            </td>
                            <td data-lable="Informações">
                            <button class="open-modal botao-listar btn-modal" data-product-id="<?= $linha['codAdm'] ?>"><i class="fa-solid fa-eye"></i></button>
                                <div class="fade hide" data-product-id="<?= $linha['codAdm'] ?>"></div>
                                <div class="modal hide" data-product-id="<?= $linha['codAdm'] ?>">

                                    <div class="modal-header">
                                        <h2>Informações de Adm</h2>
...
<script>
    $(document).ready(function() {
        $(".open-modal").click(function() {
            ...
            $.ajax({
                url: `getAdmInfo.php?codAdm=${productId}`,
                type: "GET",
                dataType: "json",
                success: function(productInfo) {
                    ...
                    if (productInfo.error) {
                        ...
                        modalBody.html(`<p><strong>ID </strong>: ${productInfo.codAdm}</p>
